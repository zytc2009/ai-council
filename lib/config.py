...
class AgentConfig:
    ...
    def validate(self, agent_id: str) -> None:
        ...
        # 3. 输出文件模式：命令包含 {output_file}，prompt 通过 stdin 或参数传递
        is_file_mode = "{prompt_file}" in self.command or "{prompt_content}" in self.command
        is_stdin_mode = " -p -" in self.command or " -q -" in self.command or " -" in self.command.rstrip()
        is_output_file_mode = "{output_file}" in self.command
        if not is_file_mode and not is_stdin_mode and not is_output_file_mode:
            raise ValueError(f"Agent '{agent_id}' command must contain {{prompt_file}} or use stdin mode (e.g., -p -, -q -, trailing -) or output file mode (e.g., -o {{output_file}})")
        if self.timeout <= 0:
